...
namespace cv19ResSupportV3.Tests.V3.E2ETests
{
    ...
    public class GetHelpRequests : IntegrationTests<Startup>
    {
        ...
        {
            var residentEntity = EntityHelpers.createResident(44);
            DatabaseContext.ResidentEntities.Add(residentEntity);
            var helpRequestEntity = EntityHelpers.createHelpRequestEntity(234, 44);
            DatabaseContext.HelpRequestEntities.Add(helpRequestEntity);
            DatabaseContext.SaveChanges();
            var expectedResponse = residentEntity.ToDomain(helpRequestEntity);
            var requestUri = new Uri($"api/v3/help-requests/{helpRequestEntity.Id}", UriKind.Relative);
